	...
	function animOnScroll() {
		for (let index = 0; index < animItems.length; index++) {
			...
			if ((pageYOffset > animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight)) {
				animItem.classList.add('magic');
			} else {
				if (!animItem.classList.contains('magic-no')) {
					animItem.classList.remove('magic');
				}
			}
		}
	}
...
//SLIDERS
if($('.slider-onlineLearning').length>0){
	$('.slider-onlineLearning').slick({
		//autoplay: true,
		//infinite: false,
		dots: false,
		arrows: true,
